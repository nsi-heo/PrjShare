...
class Group {
    ...
    public function approveJoinRequest($requestId) {
        try {
            ...
            $checkStmt = $this->conn->prepare($checkQuery);
            $checkStmt->execute([$request['group_id'], $memberName]);
            $existingMember = $checkStmt->fetch(PDO::FETCH_ASSOC);
            
            if ($existingMember) {
                // Le nom existe déjà
                if ($existingMember['user_id'] === null) {
                    // CAS 1 : Membre non-lié → Le lier au compte utilisateur
                    $this->conn->beginTransaction();
                    
                    try {
                        // Mettre à jour le membre existant pour le lier au compte
                        $linkQuery = "UPDATE group_members SET user_id = ? WHERE id = ?";
                        $linkStmt = $this->conn->prepare($linkQuery);
                        $linkStmt->execute([$request['user_id'], $existingMember['id']]);
                        ...
                        return [
                            'status' => 'success', 
                            'message' => 'Demande approuvée avec succès. Le compte a été lié au membre existant "' . $memberName . '".'
                        ];
                        
                    } catch(PDOException $e) {
                        $this->conn->rollback();
                        error_log("Erreur lors de la liaison du membre: " . $e->getMessage());
                        return ['status' => 'error', 'message' => 'Erreur lors de la liaison du membre au compte'];
                    }
                    
                } else {
                    // CAS 2 : Membre déjà lié à un autre compte
                    // Rejeter la demande et informer l'utilisateur
                    $updateQuery = "UPDATE group_join_requests SET status = 'rejected', processed_at = NOW() WHERE id = ?";
                    $updateStmt = $this->conn->prepare($updateQuery);
                    $updateStmt->execute([$requestId]);
                    
                    return [
                        'status' => 'error', 
                        'message' => 'Le nom "' . $memberName . '" est déjà utilisé par un membre lié à un compte. L\'utilisateur doit s\'inscrire avec un autre nom d\'utilisateur.'
                    ];
                }
            }
            
            // CAS 3 : Nom disponible → Ajouter normalement
            $this->conn->beginTransaction();
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
                    return ['status' => 'error', 'message' => 'Ce membre existe déjà dans le groupe'];
                }
                
                return ['status' => 'error', 'message' => 'Erreur lors de l\'ajout du membre au groupe'];
            }
            ...
            return ['status' => 'error', 'message' => 'Erreur système lors du traitement de la demande'];
        }
    }
